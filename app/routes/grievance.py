...
def create_grievance(
    ...
    db.add(grievance)
    db.commit()
    send_grievance_submission_email(current_user.email, current_user.email, grievance.id)
    notify_roles(
        db,
        [UserRole.ADMIN, UserRole.EXPERT],
        "New grievance submitted",
        f'User "{current_user.email}" submitted grievance #{grievance.id}.',
        "/dashboard",
        "grievance",
    )
    db.commit()
...
def add_grievance_comment(
    ...
    if current_user.role in {UserRole.ADMIN, UserRole.EXPERT}:
        create_notification(
            ...
            grievance.user_id,
            "New grievance reply",
            f'{current_user.email} replied on grievance #{grievance.id}.',
            "/grievances",
            "grievance_comment",
        )
        if grievance.user:
        ...
        notify_roles(
            ...
            f'{current_user.email} added a reply on grievance #{grievance.id}.',
            "/dashboard",
            "grievance_comment",
